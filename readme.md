...
- **Ручное обновление таблиц:**
  ```bash
  docker exec app node -e "import('./dist/services/googleSheets.service.js').then(m => new m.GoogleSheetsService().updateAllSheets())"
  ```

## Описание

В шаблоне настроены контейнеры для postgres и приложения на nodejs. Для взаимодействия с БД используется knex.js. В контейнере app используется build для приложения на ts.
